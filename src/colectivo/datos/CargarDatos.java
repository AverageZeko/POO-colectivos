...
public class CargarDatos {
	...
	public static Map<String, Tramo> cargarTramos(String nombreArchivo, Map<Integer, Parada> paradas)
	    ...
	    InputStream inputStream = CargarDatos.class.getClassLoader()
	            .getResourceAsStream("resources/" + nombreArchivo);
	    if (inputStream == null) {
	        throw new FileNotFoundException("No fue posible encontrar " + nombreArchivo
	                + " en la carpeta resources del classpath.");
	    }
	    ...
	    try (Scanner contenidoArchivo = new Scanner(inputStream)) {
	        while (contenidoArchivo.hasNextLine()) {
	            ...
	            try {
	                ...
	                // Clave determinística (sin hashCode)
	                String tramoKey = construirClaveTramo(inicio, fin);           // ej: "44->43"
	...
	private static String construirClaveTramo(Parada origen, Parada destino) {
	    // Si quieres incluir tipo agregar "#"+tipo más adelante (ajustar en donde se use)
